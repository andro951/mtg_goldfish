...
def persistence(p):
 ...
 p.evaluate('astra.flushSave()');prefs=p.evaluate('astra.prefs');expected=checksum(p);p.reload();p.wait_for_function('()=>window.astra?.engine')
 check('reload preserves the saved game exactly',checksum(p)==expected,{'before':expected,'after':checksum(p)})
 check('reload preserves hand/rail/dock/popup preferences',p.evaluate('astra.prefs')==prefs,{'before':prefs,'after':p.evaluate('astra.prefs')})
 menu(p,'save')
 ...
 manual_controls(p,True);before=state(p,'state.players[0].life');p.locator('[data-action=resource][data-player="0"][data-field=life][data-delta="-1"]').click();p.evaluate('astra.flushSave()');menu(p,'save')
