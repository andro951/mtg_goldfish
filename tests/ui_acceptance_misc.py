...
def phase_controls(p):
 ...
 check('Plot pays its cost, exiles the card, and closes the stale hand inspector',obj(p,'Pitiless Carnage','exile') is not None and state(p,'state.stack.length')==0 and p.locator('.inspector-window').count()==0)
 click(p,'zone',extra='[data-zone=exile]');inspect(p,'Pitiless Carnage','exile');check('plotted spell cannot be cast on the same turn',p.locator('.inspector-window [data-action=cast]').count()==0)
