...
async function saveNow(){
 ...
 saveFlight=(async()=>{
  ...
  while(savedRevision!==saveRevision){
   ...
   }catch(error){if(seq!==saveSequence)continue;saveStatus.text=error.message;saveStatus.error=true;updateSaveLabel();throw error;}
  }
...
async function handleClick(event){const el=event.target.closest('[data-action]');if(!el)return;const action=el.dataset.action,id=el.dataset.id;
 ...
 if(action==='order-inspect')return inspect(id,{x:event.clientX,y:event.clientY});
...
window.astra={programs,get engine(){return g;},registry,ui,get prefs(){return prefs;},run,flushSave:saveNow,get storage(){return store;},exported,version:'1.5.0'};
