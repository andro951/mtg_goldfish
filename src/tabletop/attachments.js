import { bounds, CARD_H, round } from './geometry.js';
...
export function permanentLinks(g) {
  ...
  for(const source of g.objects('battlefield')) {
    ...
    if(partner?.zone!=='battlefield'||partner.id===source.id||!sameRef(partner.pairedWith,source)||
      source.controller!==partner.controller||!g.characteristics(source).types.includes('Creature')||
...
export function attachmentLayout(g,cards,parents=followingParents(g)) {
  ...
  function group(id){
    ...
    ids.forEach((childId,i)=>{const child=byId.get(childId),h=bounds(child).height;
      child.x=round(r.left-24*(i+1));child.y=round(r.top-24*(i+1)+h);delete child.autoArrival;
    });
